...
# This is great data cleaning!
# It might be helpful to show the data before recoding it as we don't know what the numbers mean without the lables. 

css_ghana %>% 
  select(!age_generation, !value)
# Instead of showing the whole data set I am wondering if there are other descriptive data you could show.
...
css_ghana %>% 
  ...
  head() %>% 
  datatable()
# Super cool that it is searchable! Maybe you can add more than 6 entries so you can search the whole data set

# You could add additional descriptive statistics by generation
describeBy(css_ghana[5:9], group = css_ghana$age_gen_recoded, range = FALSE)

```

## Plotting those who were concerned about climate change by age generaton
```{r Descriptives}
# This graph looks great! Love the title and axis labels. 
# One suggestion is to simplify the Legend title.
...
css_ghana %>% 
  ...
  labs(x = "Climate Change Should Be Prioritized (1 = Low, 5 = High)",
         y = "Frequency",
       fill = "Generation", # An option if you want to change your legend title
 ...
 group_by(age_gen_recoded) %>% 
  ...
  theme_minimal() +
   labs(x = "Age Generation",
        ...
        fill = "Generation", # Again an option if you want to change the legend
  ...
  scale_fill_manual(values = c("grey0",
                              ...
                              "goldenrod4")) +
  coord_flip()
# Great colors and nice work on the coord flip to make it easier to read! Would be helpful to know about the scale for the x axis
...
css_ghana %>% 
  ...
  labs(x = "Age Generation", # To add some labels
         y = "Message score",
        fill = "Generation", # Again an option if you want to change the legend
         title = "Climate Change Influence on Child Desires",
         subtitle = "(Data obtained from a survey in Ghana, West Africa)")

# This is interesting information, but I am not sure what is represented on the y axis, what do the numbers mean?
