...
library(readxl)
library(DT)
library(kableExtra) # I added this one for your table

#I had to install psych, skimr and DT, thanks for introducing me to new packages! I'll look into what they do. 
#Cool use of sys.date, I didn't know how to use it.
...
<!-- I deleted some data that was repeated, please check if that was a mistake on my part.  -->
...
css_ghana1 <- css_ghana %>% 
  select(-age_generation, -value)

#Instead of !, you can use the minus "-" sign to remove a column from your selection. I created a new object for you without those 2 columns.I hope it helps!


## Pivot Wider for Assignment

# Check the names of the variables here. It looks like you're using age_generation after removing it in the previous step (I think). 
# One suggestion for the summary table: save it as a different object so you can access it separately later.  
...
#I think maybe using group_by and summarize might help with your descriptives. 
#I Added an example here using pivot wider and kable to make it look good in your document :). I hope it helps!

css_ghana2 <- css_ghana %>% 
  clean_names() %>% 
  select(age_gen_recoded, age_generation, gender, q_16_not_parent_chid_desire) %>% 
  group_by( age_gen_recoded, gender) %>% 
  summarize("child_desire" = mean(q_16_not_parent_chid_desire)) %>% 
  pivot_wider(
    names_from = gender,
    values_from = child_desire) %>% 
  mutate(Female = round(Female, digits = 2),
...
# Nice work! This graph really portrays the objective of your paper. 
#I added a title label to your graph.
...
#Again, great work! I like that you did the coordflip. It makes the graph very easy to read.  
#I added a title label to your graph. 
...
#Great advance! I am looking forward to seeing your final presentation. You did an amazing job tidying the data and starting with your graphs and tables. It is an interesting topic too, so I am looking forward to your results. 
